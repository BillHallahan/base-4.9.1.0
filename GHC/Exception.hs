...
-}
-- G2 HACK: Typeable is auto-derived for all types, so the constraint here must be that autoderived
-- Typeable that actually exists... but then we need to do mapping between the "real" (GHC hooked in) base types
-- and the "fake" (g2) base types in from exeception.
class (BaseTy.Typeable e, Show e) => Exception e where
    toException   :: e -> SomeException
    fromException :: SomeException -> Maybe e

    toException = SomeException
    fromException (SomeException e) = case BaseTy.cast e of Base.Just x -> Just x; Base.Nothing -> Nothing
