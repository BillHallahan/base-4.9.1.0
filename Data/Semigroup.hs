...
instance Num a => Monoid (Product a) where
        mempty = Product (fromInteger (Z# 1#))
        -- By default, we would get a lazy right fold. This forces the use of a strict
        -- left fold instead.
        mconcat = List.foldl' (<>) mempty
        {-# INLINE mconcat #-}
...
instance Semigroup (Endo a) where
    ...
    stimes !n0 (Endo e) = Endo (\a0 ->
      ...
      case n0 of
        _ | n0 < (fromInteger (Z# 0#)) -> stimesEndoError
          | n0 == (fromInteger (Z# 0#)) -> a0
          | n0 == (fromInteger (Z# 1#)) -> e a0
        _ -> go n0 a0)
      where
        go !n a | n == (fromInteger (Z# 0#)) = a
        go n a = e (go (n - (fromInteger (Z# 1#))) a)
...
stimesMonoid n x0 = case compare n (fromInteger (Z# 0#)) of
  LT -> errorWithoutStackTrace "stimesMonoid: negative multiplier"
  EQ -> mempty
  GT -> f x0 n
    where
      f x y
        | even y = f (x `mappend` x) (y `quot` fromInteger (Z# 2#))
        | y == (fromInteger (Z# 1#)) = x
        | otherwise = g (x `mappend` x) (pred y  `quot` fromInteger (Z# 2#)) x
      g x y z
        | even y = g (x `mappend` x) (y `quot` fromInteger (Z# 2#)) z
        | y == (fromInteger (Z# 1#)) = x `mappend` z
        | otherwise = g (x `mappend` x) (pred y `quot` fromInteger (Z# 2#)) (x `mappend` z)
...
stimesIdempotentMonoid n x = case compare n (fromInteger (Z# 0#)) of
...
stimesIdempotent n x
  | n <= (fromInteger (Z# 0#)) = errorWithoutStackTrace "stimesIdempotent: positive multiplier expected"
  | otherwise = x
