...
approxRational rat eps  =  simplest (rat-eps) (rat+eps)
        where simplest x y | y < x      =  simplest y x
                           | x == y     =  xr
                           | x > fromInteger (Z# 0#)      =  simplest' n d n' d'
                           | y < fromInteger (Z# 0#)      =  negate (simplest' (negate n') d' (negate n) d)
                           | otherwise  =  fromInteger (Z# 0#) :% fromInteger (Z# 1#)
              ...
              simplest' n d n' d'       -- assumes 0 < n%d < n'%d'
                        | r == fromInteger (Z# 0#)     =  q :% fromInteger (Z# 1#)
                        | q /= q'    =  (q+fromInteger (Z# 1#)) :% fromInteger (Z# 1#)
                        | otherwise  =  (q*n''+d'') :% n''
